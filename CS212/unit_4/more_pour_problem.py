...
def more_pour_problem(capacities, goal, start=None):
    ...
    def multysuccessors(state):
        state_action_dict = {}
        for i, amount in enumerate(state):
            # amount shows level
            if amount != capacities[i]:
                final_state = list(state)
                final_state[i] = capacities[i]
                state_action_dict[tuple(final_state)] = ('fill', i)
                
            if amount != 0:
                final_state = list(state)
                final_state[i] = 0
                state_action_dict[tuple(final_state)] = ('empty', i)
                
        for from_, to_ in itertools.permutations(range(len(state)), 2):
            final_state = list(state)
            from_isnt_empty = (final_state[from_] != 0)
            to_isnt_full = (final_state[to_] != capacities[to_])
            if from_isnt_empty and to_isnt_full:
                amount_to_pour = min(
                    capacities[to_] - final_state[to_],
                    final_state[from_]
                )            
                final_state[from_] -= amount_to_pour
                final_state[to_] += amount_to_pour
                state_action_dict[tuple(final_state)] = ('pour', from_, to_)
    ...
    return shortest_path_search(start, multysuccessors, is_goal)
