...
def subway(**lines):
    ...
    for line_name, line in lines.items():
        ...
        for i, stop in enumerate(stations):
            if i-1 >= 0:
                neighbors[stop].append((stations[i-1], line_name))
            if i+1 < len(stations):
                neighbors[stop].append((stations[i+1], line_name))
...
def ride(here, there, system=boston):
    ...
    def metrosuccessors(state):
        result = {}
        neighbors = system[state]
    ...
    return shortest_path_search(here, metrosuccessors, goal)

def longest_ride(system):
    """"Return the longest possible 'shortest path' 
    ride between any two stops in the system."""
    longest = ()
    pairs = itertools.permutations(system.keys(), 2)
    for i in pairs:
        path = ride(*i)
